...
object SEATS {
  ...
  class Flight(id: Int) extends Compartment {
    ...
    val flying_to = Relationship("flying_to").from[PlaneInComission](0 To *).to[DepartureAirport](1)
    val depart_from = Relationship("depart_from").from[PlaneInComission](0 To *).to[DepartureAirport](1)
    val owns = Relationship("owns").from[Airline](1).to[PlaneInComission](1 To *)
    val reserves = Relationship("reserves").from[Customer](1 To *).to[PlaneInComission](1 To *)
    val is_frequent_flyer = Relationship("is_frequent_flyer").from[Customer](0 To *).to[Airline](0 To 1)
