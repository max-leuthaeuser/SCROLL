...
object RunnerApp {
  def main(args: Array[String]): Unit = {
    ...
    ResultFormatFactory.getInstance(ResultFormatType.SCSV, s"benchmark_$timeStamp.csv").writeOut(results)
    ResultFormatFactory.getInstance(ResultFormatType.JSON, s"benchmark_$timeStamp.json").writeOut(results)
  }
}