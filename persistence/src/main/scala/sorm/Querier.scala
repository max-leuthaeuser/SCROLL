...
class Querier[T <: AnyRef : WeakTypeTag](query: Query, connector: Connector) {
  ...
  = {
    ...
    = {
      ...
      f match {
        ...
        case _ =>
          pvo match {
            case (p, v, o) => Path.where(query.mapping, p, v, o)
          }
      }
    }
    where(queryWhere(f))
...
object Querier {
  def apply[T <: AnyRef : WeakTypeTag](mapping: EntityMapping, connector: Connector)
