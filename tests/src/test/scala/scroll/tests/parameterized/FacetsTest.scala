  ...
  test("Filtering for facets") {
    forAll(PARAMS) { (c: Boolean, cc: Boolean) =>
      ...
      new CompartmentUnderTest(c, cc) {
        ...
        someCoreA5 <+> Blue
        someCoreA6 <+> Blue
        roleQueries.all { c: CoreA => c.hasFacets(Red) } should contain theSameElementsAs Set(someCoreA1, someCoreA2, someCoreA3)
        roleQueries.all { c: CoreA => c.hasSomeFacet(Red) } should contain theSameElementsAs Set(someCoreA1, someCoreA2, someCoreA3)
        roleQueries.all { c: CoreA => c.hasFacets(Blue) } should contain theSameElementsAs Set(someCoreA4, someCoreA5, someCoreA6)
        roleQueries.all { c: CoreA => c.hasSomeFacet(Blue) } should contain theSameElementsAs Set(someCoreA4, someCoreA5, someCoreA6)
        roleQueries.all { c: CoreA => c.hasSomeFacet(Red, Blue) } should contain theSameElementsAs Set(someCoreA1, someCoreA2, someCoreA3, someCoreA4, someCoreA5, someCoreA6)
