...
final case class FormalCROM[NT >: Null <: AnyRef, RT >: Null <: AnyRef, CT >: Null <: AnyRef, RST >: Null <: AnyRef](
  ...
  def axiom1: Boolean = FormalUtils.all(rt.map(r => FormalUtils.any(nt.concat(ct).map(t => fills.contains((t, r))))))

  def axiom2: Boolean = FormalUtils.all(ct.map(c => parts(c).nonEmpty))

  def axiom3: Boolean = FormalUtils.all(rt.map(r => ct.filter(parts(_).contains(r)).lengthCompare(1) == 0))
